...
HWND hEdit; // Handle para a área de texto
HWND hStatus; // Handle para a barra de status
WNDPROC oldEditProc;
HWND hListBox;  // Handle para a lista de erros
HWND hBtnCompilar;  // Handle para o botão de compilar
...
// Adicione esta função de subclasse do Edit
...
// Função para tratar o menu "Abrir"
...
int nivel;
ListaTokens lista_expressao = {0};  // para coleta durante análise
ListaTokens exp_posfix = {0};       // para resultado da conversão
...
void erro() {
    char mensagem[256];
    sprintf(mensagem, "Erro na linha: %d", linhas);

    #ifdef GUI_MODE
        Se estiver no modo GUI, envia para a listbox
        SendMessage(hListBox, LB_ADDSTRING, 0, (LPARAM)mensagem);
    #else
        // Se estiver no modo console, mantém o comportamento original
        printf("\n\nErro!\n\n");
        printf("\n\nErro na linha: %d\n\n", linhas);
    #endif
}
...
int main(int argc, char* argv[]) {
    if (argc > 1) {  // Se tiver argumentos, usa modo console
        ...
        return 0;
    } else {  // Se não tiver argumentos, usa modo gráfico
